...
class ConfigBeanTest extends Unit
{
    ...
    public function testSetterAndGetterClientId()
    {
        $this->specify('Checks setter and getter for "client_id" property.', function(){
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setClientId('my-id'));
    ...
    public function testSetterAndGetterClientSecret()
    {
        $this->specify('Checks setter and getter for "client_secret" property.', function(){
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setClientSecret('my-secret'));
    ...
    public function testSetterAndGetterApplicationName()
    {
        $this->specify('Checks setter and getter for "application name" property.', function(){
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setAppName('my-app-name'));
    ...
    public function testSetterAndGetterBrand()
    {
        $this->specify('Checks setter and getter for "brand" property.', function() {
            ...
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setBrand(new Brand(['key' => 'my-brand-key', 'name' => 'my-brand-name'])));
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Brand', $this->config->getBrand());
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setBrand(new Brand(['key' => 'my-brand-key-2', 'name' => 'my-brand-name-2'])));
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Brand', $this->config->getBrand());
    ...
    public function testSetterAndGetterOpi()
    {
        $this->specify('Checks setter and getter for "opi" property.', function(){
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setOpi('my-opi-code'));
    ...
    public function testSetterAndGetterHosts()
    {
        $this->specify('Checks setter and getter for "hosts" property.', function(){
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setHosts([
                new Host(['id' => 1, 'url' => 'www.foo.com']),
                new Host(['id' => 2, 'url' => 'www.bar.com']),
            ]));
            $this->assertCount(2, $this->config->getHosts());
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->addHost(new Host(['id' => 3, 'url' => 'www.fuu.com'])));
            $this->assertCount(3, $this->config->getHosts());
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Host', $this->config->getHost(1));
            $this->assertEquals(1, $this->config->getHost(1)->getId());
            $this->assertFalse($this->config->getHost(4));
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->addHost(new Host(['id' => 3, 'url' => 'www.fuu-2.com'])));
            $this->assertCount(3, $this->config->getHosts());
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Host', $this->config->getHost(3));
    ...
    public function testSetterAndGetterEndpoints()
    {
        $this->specify('Checks setter and getter for "endpoints" property.', function(){
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setEndPoints([
                new EndPoint(['id' => 1, 'url' => 'www.foo.com']),
                new EndPoint(['id' => 2, 'url' => 'www.bar.com']),
            ]));
            $this->assertCount(2, $this->config->getEndPoints());
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->addEndPoint(new EndPoint(['id' => 3, 'url' => 'www.fuu.com'])));
            $this->assertCount(3, $this->config->getEndPoints());
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\EndPoint', $this->config->getEndPoint(1));
            $this->assertEquals(1, $this->config->getEndPoint(1)->getId());
            $this->assertFalse($this->config->getEndPoint(4));
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->addEndPoint(new EndPoint(['id' => 3, 'url' => 'www.fuu-ep-2.com'])));
            $this->assertCount(3, $this->config->getEndPoints());
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\EndPoint', $this->config->getEndPoint(3));
    ...
    public function testSetterAndGetterEntryPoints()
    {
        $this->specify('Checks setter and getter for "entry points" property.', function(){
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setEntryPoints([
            ...
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->addEntryPoint(new EntryPoint(['id' => 3, 'promotion_id' => 'promo3', 'prizes' => []]), true));
            $this->assertCount(3, $this->config->getEntryPoints()['entry_points']);
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\EntryPoint', $this->config->getEntryPoint(1));
            $this->assertEquals(1, $this->config->getEntryPoint(1)->getId());
            $this->assertFalse($this->config->getEntryPoint(4));

            // Default entry point.
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\EntryPoint', $this->config->getEntryPoint());
            $this->assertEquals(3, $this->config->getEntryPoint()->getId());

            // Checks if an existing entry point is updated.
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->addEntryPoint(new EntryPoint(['id' => 3, 'promotion_id' => 'promo3_2', 'prizes' => []]), true));
            $this->assertCount(3, $this->config->getEntryPoints()['entry_points']);
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\EntryPoint', $this->config->getEntryPoint(3));
    ...
    public function testSetterAndGetterRedirects()
    {
        $this->specify('Checks setter and getter for "redirects" property.', function(){
            ...
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setRedirects([
            ...
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->addRedirect(new RedirectUrl(['type' => 'type-4', 'url' => 'www.type-4_1.com', 'is_default' => false])));
    ...
    public function testSetterAndGetterApi()
    {
        $this->specify('Checks setter and getter for "api" property.', function(){
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->setApis([
            ...
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Api', $this->config->getApi('api-1'));
            $this->assertEquals('api-1', $this->config->getApi('api-1')->getName());
            $this->assertFalse($this->config->getApi('api-99'));
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->addApi(new Api(['name' => 'api-3', 'base_url' => 'www.api-3.com', 'endpoints' => ['a3' => 'aaa3', 'b3' => 'bbb3']])));
            $this->assertCount(3, $this->config->getApis());
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $this->config->addApi(new Api(['name' => 'api-1', 'base_url' => 'www.api-111.com', 'endpoints' => ['a1' => 'aaa1', 'b1' => 'bbb1']])));
            $this->assertCount(3, $this->config->getApis());
            $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Api', $this->config->getApi('api-1'));
