 ...
 * @category UnitTest
 */
class OAuthConfigFactoryServiceTest extends TestCase
{
    use Specify;
    ...
    /**
     * @param \Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config $config
     */
    private function verificationSteps($config)
    {
        $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Config', $config);
        ...
        $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Brand', $config->getBrand());
        ...
        $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Host', $config->getHost('my-host-2'));
        $this->assertEquals('//www.foo-host-2.com', $config->getHost('my-host-2')->getUrl());

        $this->assertCount(5, $config->getRedirects());
        $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\RedirectUrl', $config->getRedirect('postLogin'));
        ...
        $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\EntryPoint', $config->getEntryPoint('2222222-main'));
        $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\EntryPoint', $config->getEntryPoint());
        ...
        $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\EndPoint', $config->getEndPoint('cancel_url'));
        ...
        $this->assertInstanceOf('\Genetsis\DruID\Core\OAuth\Beans\OAuthConfig\Api', $config->getApi('api.activityid'));
